...
allowed-tools:
  ...
  - Bash(chmod:+x $HOME/.claude/hooks/session-prompt-hook.sh)
  - Bash(test:-* $HOME/.claude/hooks/session-prompt-hook.sh)
  - Read($HOME/.claude/hooks/session-prompt-hook.sh)
  - Write($HOME/.claude/hooks/session-prompt-hook.sh)
  - Bash(which:jq)
...
This hook captures user prompts during a Claude Code session and saves them to `TARGET_DIR="/tmp/claude-sessions/"`.
...
1. **Copy hook script to Claude Code config directory**
   - Source: `plugins/status-line/hooks/session-prompt-hook.sh`
...
2. **Configure Claude Code to use the hook**
   ...
   - Replace `$HOME` with the actual `$HOME` environmental variable.
   - If settings.json already has other content in the hooks array, append this configuration
...
4. **Provide usage instructions**
   ...
   - Explain the `$TARGET_DIR/prompts-{session_id}.txt` file location
...
1. **Basic installation**:
   ...
   To enable the hook, run the install-prompt-hook command.
...
- The hook captures prompts to `$TARGET_DIR/prompts-{session_id}.txt` (persist until reboot)
